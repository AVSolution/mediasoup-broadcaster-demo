...
CRenderGDI::CRenderGDI(eRenderType renderType)
  : m_nWidth(0), m_nHeight(0), m_wnd(nullptr), m_bThreadRuning(false), m_nFrameCount(0),
    m_bEnableTipInfo(false), m_pImageBk(nullptr), m_renderType(renderType)
{
}
...
CRenderGDI::CRenderGDI(void* pHandle, int nWidth, int nHeight)
  : m_nWidth(nWidth), m_nHeight(nHeight), m_wnd(pHandle)
{
}
...
bool CRenderGDI::showvideo(
  void* pHandle, std::shared_ptr<uint8_t> buffer, int nBufferLen, int nWidth, int nHeight)
{
	if (
	  pHandle && ((nBufferLen == nWidth * nHeight * 3 && eRenderType_RGB24 == m_renderType) ||
